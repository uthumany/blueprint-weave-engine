...
export const ctxScreenshot = createServerFn({ method: "POST" })
  .inputValidator((d: { target: string; fullPage?: boolean }) =>
    z.object({ target: z.string().min(1).max(2048), fullPage: z.boolean().optional() }).parse(d),
  )
  .handler(async ({ data }) =>
...
export const ctxCrawlSite = createServerFn({ method: "POST" })
  .inputValidator((d: { url: string; maxPages?: number }) =>
    z.object({ url: urlSchema, maxPages: z.number().int().min(1).max(50).optional() }).parse(d),
  )
  .handler(async ({ data }) =>
...
export const ctxCrawlSitemap = createServerFn({ method: "POST" })
  .inputValidator((d: { domain: string; maxLinks?: number }) =>
    z.object({ domain: z.string().min(1).max(255), maxLinks: z.number().int().min(1).max(5000).optional() }).parse(d),
  )
  .handler(async ({ data }) =>
    (await ctx()).crawlSitemap(
      data.domain.replace(/^https?:\/\//i, "").replace(/\/.*$/, ""),
...
export const ctxWebSearch = createServerFn({ method: "POST" })
  .inputValidator((d: { query: string; numResults?: number }) =>
    z.object({ query: z.string().min(1).max(500), numResults: z.number().int().min(1).max(20).optional() }).parse(d),
  )
